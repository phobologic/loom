...
class TestCharactersPage:
    ...
    async def test_page_shows_create_form_when_no_character(
    ...
    async def test_page_hides_create_form_when_has_character(
        self, client: AsyncClient, db: AsyncSession
    ) -> None:
        game_id = await self._setup(client, db)
        await client.post(
    ...
    async def test_all_members_see_all_characters(
        self, client: AsyncClient, db: AsyncSession
    ) -> None:
        game_id = await self._setup(client, db)
    ...
    async def test_page_requires_auth(self, client: AsyncClient, db: AsyncSession) -> None:
        game_id = await self._setup(client, db)
...
class TestCreateCharacter:
    ...
    async def test_create_succeeds(self, client: AsyncClient, db: AsyncSession) -> None:
        game_id = await self._setup(client, db)
        ...
        assert response.status_code == 303
        chars = await _get_characters(db, game_id)
        assert len(chars) == 1
        assert chars[0].name == "Aria"
        assert chars[0].description == "A scholar of the old ways"
        assert chars[0].notes == "Curious"

    async def test_create_with_name_only(self, client: AsyncClient, db: AsyncSession) -> None:
        game_id = await self._setup(client, db)
        response = await client.post(
            f"/games/{game_id}/characters",
    ...
    async def test_one_character_per_player(self, client: AsyncClient, db: AsyncSession) -> None:
        game_id = await self._setup(client, db)
        await client.post(
            f"/games/{game_id}/characters",
    ...
    async def test_empty_name_rejected(self, client: AsyncClient, db: AsyncSession) -> None:
        game_id = await self._setup(client, db)
        response = await client.post(
            f"/games/{game_id}/characters",
    ...
    async def test_nonmember_blocked(self, client: AsyncClient, db: AsyncSession) -> None:
        game_id = await self._setup(client, db)
        await _login(client, 2)
        response = await client.post(
            f"/games/{game_id}/characters",
    ...
    async def test_blocked_during_setup(self, client: AsyncClient, db: AsyncSession) -> None:
        await _login(client, 1)
        game_id = await _create_game(client)
        response = await client.post(
            f"/games/{game_id}/characters",
    ...
    async def test_multiple_players_can_each_create_one(
        self, client: AsyncClient, db: AsyncSession
    ) -> None:
        game_id = await self._setup(client, db)
        await client.post(
...
class TestEditCharacter:
    async def _setup_with_char(self, client: AsyncClient, db: AsyncSession) -> tuple[int, int]:
    ...
    async def test_edit_page_loads_for_owner(self, client: AsyncClient, db: AsyncSession) -> None:
        game_id, char_id = await self._setup_with_char(client, db)
        response = await client.get(f"/games/{game_id}/characters/{char_id}/edit")
        assert response.status_code == 200
        assert "Aria" in response.text
        assert "Original desc" in response.text

    async def test_owner_can_update(self, client: AsyncClient, db: AsyncSession) -> None:
        game_id, char_id = await self._setup_with_char(client, db)
        response = await client.post(
    ...
    async def test_organizer_cannot_edit_others(
        self, client: AsyncClient, db: AsyncSession
    ) -> None:
        ...
        await _login(client, 1)
        bob_chars = [c for c in await _get_characters(db, game_id) if c.name == "Bob Char"]
    ...
    async def test_player_cannot_edit_others(self, client: AsyncClient, db: AsyncSession) -> None:
        game_id, char_id = await self._setup_with_char(client, db)
        await _add_member(db, game_id, 2)
        await _login(client, 2)
    ...
    async def test_empty_name_rejected_on_edit(self, client: AsyncClient, db: AsyncSession) -> None:
        game_id, char_id = await self._setup_with_char(client, db)
        response = await client.post(
            f"/games/{game_id}/characters/{char_id}/edit",
            data={"name": "  ", "description": "", "notes": ""},
    ...
    async def test_nonmember_cannot_see_edit_page(
        self, client: AsyncClient, db: AsyncSession
    ) -> None:
        game_id, char_id = await self._setup_with_char(client, db)
        await _login(client, 2)
    ...
    async def test_char_not_found_returns_404(self, client: AsyncClient, db: AsyncSession) -> None:
        game_id, _ = await self._setup_with_char(client, db)
