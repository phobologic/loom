...
async def _get_safety_tools(game_id: int) -> list[GameSafetyTool]:
    async with _test_session_factory() as db:
...
async def _get_safety_tools_prompt_id(game_id: int) -> int:
    async with _test_session_factory() as db:
        result = await db.execute(
...
class TestSafetyToolsPage:
    ...
    async def test_shows_lines_and_veils(self, client: AsyncClient) -> None:
        game_id = await self._setup(client)
...
class TestAddSafetyTool:
    ...
    async def test_organizer_can_add_line(self, client: AsyncClient) -> None:
        game_id = await self._setup(client)
        ...
        assert response.status_code == 303
        tools = await _get_safety_tools(game_id)
        assert len(tools) == 1
        assert tools[0].kind == SafetyToolKind.line
        assert tools[0].description == "No gratuitous violence"

    async def test_player_can_add_veil(self, client: AsyncClient) -> None:
        game_id = await self._setup(client)
        await _add_member(game_id, 2)
        await _login(client, 2)
        response = await client.post(
    ...
    async def test_multiple_members_can_add(self, client: AsyncClient) -> None:
        game_id = await self._setup(client)
        await _add_member(game_id, 2)
        await client.post(
            f"/games/{game_id}/safety-tools/add",
    ...
    async def test_empty_description_rejected(self, client: AsyncClient) -> None:
        game_id = await self._setup(client)
        response = await client.post(
            f"/games/{game_id}/safety-tools/add",
    ...
    async def test_nonmember_cannot_add(self, client: AsyncClient) -> None:
        game_id = await self._setup(client)
        await _login(client, 2)  # Bob is not a member
        response = await client.post(
            f"/games/{game_id}/safety-tools/add",
...
class TestDeleteSafetyTool:
    async def _setup_with_tool(
        self, client: AsyncClient, kind: str = "line", description: str = "No violence"
    ) -> tuple[int, int]:
    ...
    async def test_owner_can_delete_own(self, client: AsyncClient) -> None:
        game_id, tool_id = await self._setup_with_tool(client)
        response = await client.post(
    ...
    async def test_organizer_can_delete_any(self, client: AsyncClient) -> None:
        game_id, _ = await self._setup_with_tool(client)
        await _add_member(game_id, 2)
        await _login(client, 2)
        await client.post(
            f"/games/{game_id}/safety-tools/add",
        ...
        tools = await _get_safety_tools(game_id)
        bob_tool = next(t for t in tools if t.description == "Bob's veil")

        # Alice (organizer) deletes Bob's tool
        await _login(client, 1)
        response = await client.post(
            f"/games/{game_id}/safety-tools/{bob_tool.id}/delete",
            follow_redirects=False,
        )
        assert response.status_code == 303
        tools = await _get_safety_tools(game_id)
    ...
    async def test_player_cannot_delete_others(self, client: AsyncClient) -> None:
        game_id, tool_id = await self._setup_with_tool(client)
        await _add_member(game_id, 2)
    ...
    async def test_nonmember_cannot_delete(self, client: AsyncClient) -> None:
        game_id, tool_id = await self._setup_with_tool(client)
        await _login(client, 2)  # Bob is not a member
        response = await client.post(
            f"/games/{game_id}/safety-tools/{tool_id}/delete",
    ...
    async def test_tool_not_found_in_wrong_game(self, client: AsyncClient) -> None:
        await _login(client, 1)
        game_id1, tool_id = await self._setup_with_tool(client)
...
class TestSafetyToolsInSession0Wizard:
    async def _setup(self, client: AsyncClient) -> tuple[int, int]:
        ...
        prompt_id = await _get_safety_tools_prompt_id(game_id)
        return game_id, prompt_id

    async def test_safety_tools_prompt_is_seeded(self, client: AsyncClient) -> None:
        game_id, prompt_id = await self._setup(client)
        assert prompt_id is not None

    async def test_safety_tools_page_shows_structured_ui(self, client: AsyncClient) -> None:
        game_id, prompt_id = await self._setup(client)
    ...
    async def test_mark_done_advances_wizard(self, client: AsyncClient) -> None:
        game_id, prompt_id = await self._setup(client)
    ...
    async def test_player_cannot_mark_done(self, client: AsyncClient) -> None:
        game_id, prompt_id = await self._setup(client)
        await _add_member(game_id, 2)
        async with _test_session_factory() as db:
            result = await db.execute(select(Session0Prompt).where(Session0Prompt.id == prompt_id))
    ...
    async def test_mark_done_rejects_non_safety_tools_prompt(self, client: AsyncClient) -> None:
        ...
        response = await client.post(
            f"/games/{game_id}/session0/{regular_prompt.id}/mark-done",
            follow_redirects=False,
        )
